...
public class TicketNotificationService {
    ...
    public void notifyComment(Ticket ticket, User author, String content) {
        ...
        while (matcher.find())
            userRepository.findByEmail(matcher.group(1).toLowerCase()).filter(user -> isProjectUser(ticket, user))
                    .ifPresent(recipients::add);
        recipients.stream().filter(user -> !user.getId().equals(author.getId())).forEach(
                user -> save(user, "TICKET_COMMENT", ticket, author.getName() + " commented on " + ticket.getCode()));
    }
    ...
    public void notifyStatusChange(Ticket ticket, User actor, TicketStatus oldStatus, TicketStatus newStatus) {
        ...
        recipients.stream().filter(user -> !user.getId().equals(actor.getId()))
                .forEach(user -> save(user, "TICKET_STATUS_CHANGED", ticket,
                        actor.getName() + " moved " + ticket.getCode() + " to " + newStatus.getName()));
    }
    ...
    private boolean isProjectUser(Ticket ticket, User user) {
        return ticket.getProject().getOwner().getId().equals(user.getId())
                || projectUserRepository.existsByProjectIdAndUserId(ticket.getProject().getId(), user.getId());
    }
